...
export function spawnEnemy(entities, playerRef, x = 200, y = 100, type = 'Slime') {
  if (type === 'Slime') {
    const rand = Math.random();
    if (rand < 0.5) entities.push(new GreenSlime(x, y, playerRef));
    else if (rand < 0.8) entities.push(new BlueSlime(x, y, playerRef));
    else entities.push(new RedSlime(x, y, playerRef));
  }
}
